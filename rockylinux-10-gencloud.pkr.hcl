...
source "qemu" "rockylinux-10-gencloud-x86-64" {
  ...
  boot_command       = var.gencloud_boot_command_rocky_10_x86_64
  ...
  vm_name            = "Rocky-10-GenericCloud-${formatdate("YYYYMMDD", timestamp())}.x86_64.qcow2"
  cpu_model          = "host"
  cpus               = var.cpus
  efi_boot           = true
...
source "qemu" "rockylinux-10-gencloud-aarch64" {
  iso_url            = local.iso_url_rocky_10_aarch64
  iso_checksum       = local.iso_checksum_rocky_10_aarch64
  http_directory     = var.http_directory
  shutdown_command   = var.root_shutdown_command
  ssh_username       = var.gencloud_ssh_username
  ssh_password       = var.gencloud_ssh_password
  ssh_timeout        = var.ssh_timeout
  boot_command       = var.gencloud_boot_command_rocky_10_aarch64
  boot_wait          = var.boot_wait
  accelerator        = "kvm"
  ...
  net_device         = "virtio-net"
  qemu_binary        = var.qemu_binary
  vm_name            = "Rocky-10-GenericCloud-${formatdate("YYYYMMDD", timestamp())}.aarch64.qcow2"
  cpu_model          = "host"
  cpus               = var.cpus
  qemuargs = [
    ["-boot", "strict=on"],
