#!/usr/bin/env bash
# GenericCloud build + test pipeline driver.
#
# Sequentially dispatches the two workflows that build and test GenericCloud
# (.qcow2) images for AlmaLinux and Rocky Linux:
...
#
# Run from the cloud-images repo root; gh must be authenticated.
#
# Usage:
#   tools/gencloud-build-test.sh [-v <MAJOR>] [-D <DISTRO>] [-d <YYYYMMDDhhmmss>] [-y]
#
# Options:
#   -v, --version <MAJOR>            Major version. One of:
#                                    10-kitten, 10, 9, 8. Prompts (defaulting
#                                    to 10) if not given.
#   -D, --distro <DISTRO>            Distribution: almalinux (default),
#                                    rockylinux.
...
usage() {
  cat <<'EOF'
Usage: tools/gencloud-build-test.sh [-v <MAJOR>] [-D <DISTRO>] [-d <YYYYMMDDhhmmss>] [-y]

Drives the two-step GenericCloud build + test pipeline:
  1. GenericCloud: Build Image (gencloud-build.yml)
  2. GenericCloud: Test Image  (gencloud-test.yml, per .qcow2 URL)

Options:
  -v, --version <MAJOR>          Major version: 10-kitten, 10, 9, 8.
                                 Prompted (defaulting to 10) if not given.
                                 10-kitten is not available for rockylinux.
  -D, --distro <DISTRO>          Distribution: almalinux (default), rockylinux.
...
case "${DISTRO}" in
  ...
  *) die "distro must be one of: almalinux, rockylinux (got: ${DISTRO})" ;;
esac
...
if [[ "${DISTRO}" == "rockylinux" && "${VERSION_MAJOR}" == "10-kitten" ]]; then
  die "Rocky Linux does not have a kitten variant."
fi

# --- Step 1: Build ----------------------------------------------------------

DISTRO_LABEL=$([[ "${DISTRO}" == "rockylinux" ]] && echo "Rocky Linux" || echo "AlmaLinux")
info "Step 1/2: GenericCloud: Build Image for ${DISTRO_LABEL} ${VERSION_MAJOR}"
DTS_DESC="${DATE_TIME_STAMP:-auto}"
confirm "Dispatch gencloud-build.yml (distro=${DISTRO}, version_major=${VERSION_MAJOR}, date_time_stamp=${DTS_DESC})?" \
  || die "Aborted by user."
...
# Extract per-run summary block (**Image**,
...
for rid in "${RUN_IDS_TEST[@]}"; do
  ...
  if [[ -z "${job_id}" ]]; then
    ...
    block=$(job_log "${job_id}" \
      | grep -E '^\*\*(Image|Subtype|Release|System architecture|Test)\*\*:' \
      | head -10)
...
==============================================================
GenericCloud build + test for ${DISTRO_LABEL} ${VERSION_MAJOR} complete.
