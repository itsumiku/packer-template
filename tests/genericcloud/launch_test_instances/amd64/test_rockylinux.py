...
def test_rocky_user_group(host):
    """Check if the rocky user created in a rocky group and its UID and GUID values is 1000"""
    assert host.user("rocky").exists
    assert host.group("rocky").exists
    assert host.user("rocky").uid == 1000
    assert host.user("rocky").gid == 1000


def test_rocky_sudoers_file(host):
    """Check if rocky user's sudoers file is present and correct"""
    with host.sudo():
        sudoers_file = host.file("/etc/sudoers.d/90-cloud-init-users")
        sudoers_file.contains("rocky ALL=(ALL) NOPASSWD:ALL")
...
def test_instance_ssh_pub_key(host):
    ...
    authorized_keys = host.file("/home/rocky/.ssh/authorized_keys").content_string
...
def test_network_is_working(host):
    ...
    rockylinux = host.addr("rockylinux.org")
    assert rockylinux.is_resolvable
    assert rockylinux.port(443).is_reachable
